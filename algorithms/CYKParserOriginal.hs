...
--------------------------------------------------
-- CYK parsing as described in section 7.3


module CYKParserOriginal (parse) where

import Grammar
import OrdSet
import OrdMap

err str = error ("CYKParserOriginal: " ++ str)
...
parse (grammar@(_, start, terminal, productions) :: Grammar c t) 
  ...
  where
    ...
    product acell bcell = makeMapWith (++) [ (c, [c:^[atree,btree]]) |
