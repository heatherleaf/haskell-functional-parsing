...
-- which means that LR parsing is possible


module CollectingParserOptimized (CollectingParser, 
				  grammarSize, 
				  grammarInfo, 
				  printGrammar) where
...
-- import qualified ChartParserOptimized as PRS (parse)
import qualified LRParserT as PRS (parseWith)

err str = error ("CollectingParserOptimized: " ++ str)


--------------------------------------------------
-- when LR parsing, the grammar has to be augmented

augmentAndParse :: Ord s => Grammar Int s -> [s] -> [ParseTree Int s]
-- augmentAndParse = PRS.parse               -- when Chart parsing
augmentAndParse = PRS.parseWith (-1)      -- when LR parsing
...
instance Ord s => Parser (CollectingParser s) s where
  ...
  parseFull (P start empties collect sem) = parseInput
    where grammar           = makeGrammar start grammar'
	  ...
	  parsePRS          = augmentAndParse grammar
          parseInput []     = empties
	  parseInput input  = concatMap sem' (parsePRS input)
